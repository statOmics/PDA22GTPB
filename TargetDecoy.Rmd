...
## Why are statistics out of the window
```{r}
library(TargetDecoy)
library(tidyverse)
data("ModSwissXT")
...
- E-value we expect for a random spectrum and candidate peptide.
...
$$f(x) = \pi_b \mathbin{\color{red}{ f_b(x)}}+(1-\pi_b)\mathbin{\color{blue}{ f_g(x)}},$$
What is the posterior error probability (PEP) or probability that a PSM with a score is a bad hit. 

$$
\text{PEP(x)} = \frac{\pi_b f_b(x)}{f(x)}
$$
...
$$
\begin{array}{lcl}
\text{FDR}(x\geq t)&=& \frac{\pi_b\int\limits_{x=t}^{+\infty}f_b(x)}{\int\limits_{x=t}^{+\infty}f(x)}\\
...
and we can also estimate the probability on a target above the threshold empirically:
